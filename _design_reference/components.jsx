...
// Lucide-style icons as inline SVGs (minimal, monoline)
...
const Avatar = ({ name, size = 'md', accent = false }) => {
  const cls = ['avatar', size === 'lg' ? 'lg' : '', accent ? 'accent' : ''].filter(Boolean).join(' ');
...
const Drawer = ({ open, onClose, title, sub, children, footer }) => {
  React.useEffect(() => {
    if (!open) return;
    ...
    <>
      ...
      <aside className={`drawer ${open ? 'open' : ''}`} aria-hidden={!open}>
        <header className="drawer-h">
          ...
          <button className="btn icon ghost" onClick={onClose} aria-label="סגירה">
            <Icon name="x" size={15} />
          </button>
        </header>
...
const Sidebar = ({ route, onNavigate, alertsCount, compact, agent }) => {
  const items = [
    { id: 'dashboard', label: 'דשבורד', icon: 'dashboard' },
    { id: 'clients',   label: 'לקוחות',  icon: 'users', badge: alertsCount },
  ];
  return (
    <aside className="sb">
      ...
      <nav className="sb-group">
        {items.map(it => (
          <button
            key={it.id}
            className={`sb-item ${route === it.id || (it.id === 'clients' && route === 'client') ? 'active' : ''}`}
      ...
      <div className="sb-group">
        ...
        <button className="sb-item" onClick={() => onNavigate('add-client')}>
          <Icon name="plus" size={15} />
          לקוח חדש
          <span className="kbd" style={{ marginInlineStart:'auto' }}>N</span>
        </button>
        <button className="sb-item" onClick={() => onNavigate('search')}>
      ...
      <div className="sb-group" style={{ marginTop:'auto' }}>
        ...
        <button className="sb-item" onClick={() => onNavigate('export')}>
          <Icon name="download" size={15} /> ייצוא לאקסל
        </button>
        <button className="sb-item" onClick={() => onNavigate('import')}>
...
const Empty = ({ children }) => <div className="card-empty">{children}</div>;

Object.assign(window, {
  Icon, Avatar, StatusPill, Drawer, Field, Sidebar, Topbar, Empty,
});
